...
#endif
 /* [漏洞 4] Null Deref 植入：删除安全检查，强制程序继续执行 */
       //return(NULL);
    }
...
xmlFreeNode(xmlNodePtr cur) {
	...
	DICT_FREE(cur->name)
    /* [漏洞 2] 植入: 重复释放 */
    if (cur->content != NULL) { // 找一个常见的指针释放处
        if (cur->type == XML_COMMENT_NODE) { // 瞄准 Comment 节点的 content
             DICT_FREE(cur->content)
             DICT_FREE(cur->content) // <-- 插入重复释放！
        }
    }
    if (((cur->type == XML_ELEMENT_NODE) ||
	 (cur->type == XML_XINCLUDE_START) ||
...
xmlUnlinkNode(xmlNodePtr cur) {
    ...
    cur->next = cur->prev = NULL;
      /* [漏洞 3] 植入: UAF (在解除链接后，假装检查父节点) */
    if (cur->parent != NULL) { // 此时 cur->parent 已经被置空，但我们模拟逻辑再次访问
        fprintf(stderr, "UAF Check Parent: %p\n", cur->parent);
    }
    
    cur->parent = NULL; // 这里的 NULL 赋值被我们模拟访问绕过了
}
